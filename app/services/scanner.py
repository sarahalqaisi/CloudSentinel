...
from app.services.policies import evaluate, policy_resource_types

WEIGHTS = {"critical": 20, "high": 12, "medium": 6, "low": 2, "info": 0.5}
RISK = {"critical": 95, "high": 80, "medium": 55, "low": 25, "info": 5}
...
def _fingerprint(policy_key: str, address: str, evidence: dict) -> str:
    raw = json.dumps([policy_key, address, evidence], sort_keys=True, default=str).encode()
    return hashlib.sha256(raw).hexdigest()


def run_scan(db: Session, *, name: str, source_name: str, resources: list[ParsedResource], actor: str = "analyst") -> Scan:
    ...
    for policy in policies:
        ...
        if policy.scope == "scan":
            result = evaluate(policy, stored[0] if stored else {"configuration": {}}, stored)
            if result:
                ...
                findings.append(
                    Finding(
                        ...
                        fingerprint=_fingerprint(policy.policy_key, address, result),
                        risk_score=RISK.get(policy.severity, 50),
                    )
                )
            continue
        ...
        for resource in stored:
            if resource_types and resource.resource_type not in resource_types:
                continue
            result = evaluate(policy, resource, stored)
            if not result:
                continue
            finding = Finding(
                scan_id=scan.id,
                ...
                fingerprint=_fingerprint(policy.policy_key, resource.address, result),
                risk_score=RISK.get(policy.severity, 50),
            )
